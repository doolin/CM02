...
if (!controlData.parts || !Array.isArray(controlData.parts)) {
  throw new Error("cm02-control.json is missing or malformed: expected .parts array");
}
...
function extractMethodByName(parts, methodName) {
  ...
  const objects = [];
  function walk(part) {
    if (part.prose) objects.push(part.prose.replace(/\n\n/g, "; "));
...
function generatePdf(input = {}) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      displayTitle: true,
      info: {
        Title: "CM-02 Baseline Configuration Assessment",
        Author: "NIST SP 800-53 Rev 5 CM-02 Form Generator",
        Subject:
          "NIST SP 800-53 Rev 5 CM-02 Baseline Configuration control assessment form",
    ...
    // Row data — matches 800-53A assessment procedure structure
    const odp01 = controlData.params[0];
    const odp02 = controlData.params[1];
    const objectives = extractParts(controlData.parts, "assessment-objective");

    const rows = [
      {
        section: "CM-02_ODP[01]",
        nist: odp01.guidelines[0].prose,
        response: input.frequency || "",
      },
      {
        section: "CM-02_ODP[02]",
        nist: odp02.guidelines[0].prose,
        response: input.circumstances || "",
      },
      {
        section: "CM-02a.[01]",
        nist: objectives.find((o) => o.id === "cm-2_obj.a-1")?.prose || "",
        response: input.objA01 || "",
      },
      {
        section: "CM-02a.[02]",
        nist: objectives.find((o) => o.id === "cm-2_obj.a-2")?.prose || "",
        response: input.objA02 || "",
      },
      {
        section: "CM-02b.01",
        nist: substituteParams(
          objectives.find((o) => o.id === "cm-2_obj.b.1")?.prose || "",
          params,
        ),
        response: input.objB01 || "",
      },
      {
        section: "CM-02b.02",
        nist: substituteParams(
          objectives.find((o) => o.id === "cm-2_obj.b.2")?.prose || "",
          params,
        ),
        response: input.objB02 || "",
      },
      {
        section: "CM-02b.03",
        nist: objectives.find((o) => o.id === "cm-2_obj.b.3")?.prose || "",
        response: input.objB03 || "",
      },
      {
        section: "CM-02-Examine",
        nist: `[SELECT FROM: ${extractMethodByName(controlData.parts, "EXAMINE")}]`,
        response: input.examineResponse || "",
      },
      {
        section: "CM-02-Interview",
        nist: `[SELECT FROM: ${extractMethodByName(controlData.parts, "INTERVIEW")}]`,
        response: input.interviewResponse || "",
      },
      {
        section: "CM-02-Test",
        nist: `[SELECT FROM: ${extractMethodByName(controlData.parts, "TEST")}]`,
