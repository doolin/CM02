...
const controlData = require(path.join(__dirname, '..', 'data', 'cm02-control.json'));
...
function substituteParams(prose, params) {
  return prose.replace(/\{\{\s*insert:\s*param,\s*([\w.-]+)\s*\}\}/g, (_, paramId) => {
...
function extractMethods(parts) {
  const methods = parts.filter(p => p.name === 'assessment-method');
  return methods.map(m => {
    const methodName = (m.props || []).find(p => p.name === 'method')?.value || '';
    const objects = [];
    function walk(part) {
      if (part.prose) objects.push(part.prose);
      if (part.parts) part.parts.forEach(walk);
    }
    if (m.parts) m.parts.forEach(walk);
    return { method: methodName, objects };
  });
}
...
function buildMethodsText() {
  const methods = extractMethods(controlData.parts);
  return methods.map(m => {
    const items = m.objects.join('; ');
    return `${m.method.toUpperCase()}: [SELECT FROM: ${items}]`;
  }).join('\n\n');
}
...
function drawColumnHeaders(doc, y) {
  ...
  const cols = [
    ...
    { label: 'RESPONSE', x: PAGE_MARGIN + COL_SECTION_W + COL_NIST_W, w: COL_RESPONSE_W },
  ];
  for (const col of cols) {
    doc.text(col.label, col.x + 4, y + 5, { width: col.w - 8 });
    if (col !== cols[0]) {
      doc.moveTo(col.x, y).lineTo(col.x, y + h).stroke(BORDER_COLOR);
    }
  }
  doc.restore();
  return y + h;
...
function generatePdf(input = {}) {
  return new Promise((resolve, reject) => {
    ...
    const params = {
      ...
      'cm-02_odp.02': input.circumstances || '[organization-defined circumstances]',
    };

    // Title
    let y = PAGE_MARGIN;
    doc.font('Times-Bold').fontSize(10).fillColor('#666666');
    doc.text('NIST SP 800-53 Rev 5 / 800-53A Rev 5', PAGE_MARGIN, y, { width: TABLE_W, align: 'center' });
    y += 18;
    doc.text('Configuration Management Family', PAGE_MARGIN, y, { width: TABLE_W, align: 'center' });
    y += 24;
    ...
    const rows = [
      {
        section: '1. Control ID',
        nist: 'CM-02',
        response: '',
      },
      {
        section: '2. Control Name',
        nist: 'Baseline Configuration',
        response: '',
      },
      {
        section: '3. System Name',
        nist: '',
